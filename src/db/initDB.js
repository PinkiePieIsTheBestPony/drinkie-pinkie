...
export const initialiseDB = () => {
    ...
    let getCounts = selectAllStatementDB("COUNT(query_id)", "p_prompts", null, null, null);
    if (getCounts == 0) {
        ...
        for (let i = 0; i < numOfResp; i++) {
            insertStatementDB("p_prompts(json_prompt, json_response, submitted_by)", triggerMesArr[i], responseMesArr[i], '113460834692268032');
        }
    }
